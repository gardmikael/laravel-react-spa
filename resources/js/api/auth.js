...
export const register = ({ email, first_name, last_name, password, password_confirmation }) => {
	return client('/api/register', { body: { email, first_name, last_name, password, password_confirmation } })
		.then(({ data: user, meta: { token } }) => {
			return { user, token }
		})
}
